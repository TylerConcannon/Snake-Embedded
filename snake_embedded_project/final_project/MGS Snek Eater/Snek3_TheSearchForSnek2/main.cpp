    ...
    while(true) {
        ...
        while(!DataCache::game_over) {
            ...
            if(!flag) {
                ...
                //Play sound
                QString endpath = "/Snek3_TheSearchForSnek2/codec.mp3";
        ...
        QString endpath = "/Snek3_TheSearchForSnek2/game_over_audio.mp3";
        DataCache::player->setMedia(QUrl::fromLocalFile(QDir::currentPath() + endpath));
        ...
        while(!DataCache::game_over) {
            ...
            if(difftime(current_time,timestamp_mine) >= 30) {
                ...
                if(mine_x.size() < 10) {
                    ...
                    QString endpath = "/Snek3_TheSearchForSnek2/exclamation.mp3";
                    DataCache::player->setMedia(QUrl::fromLocalFile(QDir::currentPath() + endpath));
                    DataCache::player->setVolume(game_volume);
                    DataCache::player->play();
                    usleep(10000);
...
//            pthread_mutex_lock(&DataCache::snek_lock);
                ...
                if(!DataCache::eaten && !DataCache::game_over) {
                    QString endpath = "/Snek3_TheSearchForSnek2/exclamation.mp3";
                    DataCache::player->setMedia(QUrl::fromLocalFile(QDir::currentPath() + endpath));
                    DataCache::player->setVolume(game_volume);
                    DataCache::player->play();
